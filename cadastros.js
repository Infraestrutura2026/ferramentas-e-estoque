          ...
          <div class="flex items-center gap-2">
            <div class="relative">
              ...
              <input type="text" value="${utils.escapeHtml(this.busca)}" placeholder="Buscar fornecedor..."
                class="pl-8 pr-3 py-2 text-sm border border-slate-700 rounded-lg bg-slate-900 text-slate-100 focus:ring-2 focus:ring-amber-500/60 outline-none w-56"
                oninput="fornecedoresModule.setBusca(this.value)">
            </div>
            <button onclick="fornecedoresModule.abrirModal()" class="px-4 py-2 text-sm bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold rounded-lg transition">
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-sm">
            <thead><tr class="bg-slate-900 border-b border-slate-700/60">
              <th class="px-4 py-3 text-left font-semibold text-slate-400">Fornecedor</th>
              <th class="px-4 py-3 text-left font-semibold text-slate-400">Categorias</th>
              <th class="px-4 py-3 text-left font-semibold text-slate-400">Contato</th>
              <th class="px-4 py-3 text-left font-semibold text-slate-400">Telefone</th>
            ...
            <tbody>
              ${pg.rows.map(f => `
                <tr class="border-b border-slate-700/40 hover:bg-slate-700/30 transition">
                  ...
                  <td class="px-4 py-3 text-slate-400">${utils.escapeHtml(f.categoria || '—')}</td>
                  <td class="px-4 py-3">
                    <p class="text-slate-300">${utils.escapeHtml(f.contato || '—')}</p>
                    <p class="text-[11px] text-slate-500">${utils.escapeHtml(f.email || '')}</p>
                  </td>
                  <td class="px-4 py-3 text-slate-400 whitespace-nowrap">${utils.escapeHtml(f.telefone || '—')}</td>
                  <td class="px-4 py-3 text-center">${utils.statusBadge(f.status || 'Ativo')}</td>
                  <td class="px-4 py-3 text-center whitespace-nowrap">
                    <button onclick="fornecedoresModule.abrirModal('${utils.escapeHtml(f.id)}')" class="text-sky-400 hover:text-sky-300 mx-1" title="Editar"><i class="fas fa-edit"></i></button>
                    <button onclick="fornecedoresModule.excluir('${utils.escapeHtml(f.id)}')" class="text-red-300 hover:text-red-200 mx-1" title="Excluir"><i class="fas fa-trash-alt"></i></button>
                  </td>
    ...
    container.innerHTML = `
      <div class="space-y-6">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          ${kpi('Pendentes', pendentes, 'text-amber-300')}
          ${kpi('Entregues', entregues, 'text-emerald-300')}
        ...
        <div class="bg-slate-800/60 rounded-xl shadow-sm border border-slate-700/60 overflow-hidden">
          <div class="p-4 flex flex-wrap items-center justify-between gap-3 border-b border-slate-700/60">
            ...
            <div class="flex items-center gap-2">
              ...
              <button onclick="pedidosModule.abrirModal()" class="px-4 py-2 text-sm bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold rounded-lg transition">
                <i class="fas fa-plus mr-1"></i> Novo
              </button>
            </div>
          </div>
          <div class="overflow-x-auto">
            <table class="w-full text-sm">
              <thead><tr class="bg-slate-900 border-b border-slate-700/60">
                ...
                <th class="px-4 py-3 text-right font-semibold text-slate-400">Valor Total</th>
                <th class="px-4 py-3 text-center font-semibold text-slate-400">Previsão</th>
                <th class="px-4 py-3 text-center font-semibold text-slate-400">Status</th>
                <th class="px-4 py-3 text-center font-semibold text-slate-400">Ações</th>
              </tr></thead>
              <tbody>
                ${pg.rows.map(p => {
                  ...
                  return `<tr class="border-b border-slate-700/40 hover:bg-slate-700/30 transition ${atrasado ? 'bg-red-500/5' : ''}">
                    ...
                    <td class="px-4 py-3 text-slate-400">${utils.escapeHtml(p.fornecedor || '—')}</td>
                    <td class="px-4 py-3 text-center font-mono">${utils.escapeHtml(p.quantidade || '—')} ${utils.escapeHtml(p.unidade || '')}</td>
                    <td class="px-4 py-3 text-right font-mono text-slate-300">R$ ${valor.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</td>
                    <td class="px-4 py-3 text-center ${atrasado ? 'text-red-300 font-bold' : 'text-slate-400'}">${utils.formatDate(p.previsaoEntrega)}</td>
                    ...
                    <td class="px-4 py-3 text-center whitespace-nowrap">
                      <button onclick="pedidosModule.abrirModal('${utils.escapeHtml(p.id)}')" class="text-sky-400 hover:text-sky-300 mx-1" title="Editar"><i class="fas fa-edit"></i></button>
                      <button onclick="pedidosModule.excluir('${utils.escapeHtml(p.id)}')" class="text-red-300 hover:text-red-200 mx-1" title="Excluir"><i class="fas fa-trash-alt"></i></button>
                    </td>
                  </tr>`;
    ...
    container.innerHTML = `
      <div class="bg-slate-800/60 rounded-xl shadow-sm border border-slate-700/60 overflow-hidden">
        <div class="p-4 flex flex-wrap items-center justify-between gap-3 border-b border-slate-700/60">
          ...
          <button onclick="usuariosModule.abrirModal()" class="px-4 py-2 text-sm bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold rounded-lg transition">
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-sm">
            <thead><tr class="bg-slate-900 border-b border-slate-700/60">
              <th class="px-4 py-3 text-left font-semibold text-slate-400">Usuário</th>
              <th class="px-4 py-3 text-left font-semibold text-slate-400">Nome</th>
              <th class="px-4 py-3 text-center font-semibold text-slate-400">Nível</th>
              <th class="px-4 py-3 text-center font-semibold text-slate-400">Ações</th>
            </tr></thead>
            <tbody>
              ${usuarios.map(u => `
                <tr class="border-b border-slate-700/40 hover:bg-slate-700/30 transition">
                  ...
                  <td class="px-4 py-3 text-slate-400">${utils.escapeHtml(u.nome)}</td>
                  <td class="px-4 py-3 text-center">
                    ${u.nivel === 'admin'
                      ? '<span class="inline-flex px-2 py-0.5 rounded-full text-xs font-bold bg-amber-500/10 text-amber-300 border border-amber-500/20"><i class="fas fa-user-shield mr-1"></i>Admin</span>'
                      : '<span class="inline-flex px-2 py-0.5 rounded-full text-xs font-bold bg-slate-900 text-slate-400 border border-slate-700"><i class="fas fa-user mr-1"></i>Operador</span>'}
                  </td>
                  <td class="px-4 py-3 text-center whitespace-nowrap">
                    <button onclick="usuariosModule.abrirModal('${utils.escapeHtml(u.id)}')" class="text-sky-400 hover:text-sky-300 mx-1" title="Editar/Alterar senha"><i class="fas fa-edit"></i></button>
                    ${u.usuario !== authModule.getCurrentUser() ? `<button onclick="usuariosModule.excluir('${utils.escapeHtml(u.id)}')" class="text-red-300 hover:text-red-200 mx-1" title="Excluir"><i class="fas fa-trash-alt"></i></button>` : ''}
                  </td>
